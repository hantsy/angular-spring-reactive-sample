...
* All users(includig anonymous users) can view post list and post details.
...
/posts/{id}|GET|{id:'1', title:'title'}| Get a post by id
/posts/{id}|PUT {title:'title'} |{id:'1', title:'title'}| Update specific post by id
/posts/{id}|DELETE|{id:'1', title:'title'}| Delete a post by id

Create a `Post` POJO class. Add Spring Data Mongo specific `@Document` annotation on this class to indicate it is a Mongo document.
...
To remove getters and setters, `toString`, `equals`, `hashCode` methods from `Post` and make the source code looks clearly, let's use Lombok specific anntations to generate these required facilities at compile time via annotation processor tooling support.

Create `PostRepository` interface for `Post` document. Make sure it is extended from `ReactiveMongoRepository`, which is the reactive variant of `MongoRepository` interface and it is ready for reactive operations.
...
Let's create `PostController` to expose RESTful APIs.
...
It is very similiar with traditional MVC `@RestController`, the difference is here we use Reactor specific `Mono` and `Flux` as return result type.
...
Now we are ready to start up the application. Before this you have to make sure there is a running MongoDB instance.

I have prepared *docker-compose.yml* to bootstrap a MongoDB instance quickly.
...
>NOTE: You can also install a local MongoDB service in your local system.

Now try to execute `mvn spring-boot:run` to start up the application.

Or click **Run** action in the context menu in your IDE.

When it is started, open another terminal window, use `curl` or `httpie` to have a try.
...
Next let's add access control to protect the `Post` APIs.

### Protect APIs

Like traditional MVC, when `spring-security-web` is existed in the classpath in a webflux application, Spring Boot will configure security automaticially.

To customize security configuration, create a `@Configuration` class.
...
To protect REST APIs, token based authentication is mostly used.
...
Currently, Spring Session provides reactive supports for Redis and MongoDB.
...
Spring Session will be enabled and backed by MongoDB automatically.
...
Declare `WebSessionIdResolver` bean to use header to resovle session id instead of cookie.
...
As you see, there is a `X-AUTH-TOKEN` header in the response headers when the authentication is sucessful.

Try to add `X-AUTH-TOKEN` to request headers and access the protected APIs.
...
Oh, it works. You can try more on other cases, such as creating and updating posts.

### Exception Handling

For traditional servlet stack, we can use `@ExceptionHandler` to handle the exceptions and convert them into http friendly messsages.
...
For the bean validations, convert the `WebExchangeBindException` to a `UnprocessableEntity` error to client, please see the complete codes of [`RestExceptionHandler`](https://github.com/hantsy/angular-spring-reactive-sample/blob/master/server/src/main/java/com/example/demo/RestExceptionHandler.java).
...
To enable guesture in Angular Material, install `hammerjs`.
...
### Generate the proejct structure
